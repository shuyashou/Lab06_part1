...
shared_ptr<demogData> readCSVLineDemog(std::string theLine) {
    ...
    raceDemogData inRD(round(FirstNation*totalPop2014),round(Asian*totalPop2014),round(Black*totalPop2014),round(Latinx*totalPop2014),round(HIPacificIsle*totalPop2014),round(
    MultiRace*totalPop2014),round(White*totalPop2014),round(WhiteNH*totalPop2014),totalPop2014);
 
    //update as needed this is lab3 version
/*
    return make_shared<demogData>(name, state, popOver65, popUnder18,
            popUnder5, totalPop2014,bachelorDegreeUp,highSchoolUp,belowPoverty,inRD);
*/
    return make_shared<demogData>(name, state, popOver65, popUnder18,
...
shared_ptr<psData> readCSVLinePolice(std::string theLine) {
    ...
    getFieldNQ(ss);//skip a clm.
    string flee =  getFieldNQ(ss);
    //finish reading other data fields and initialize police data

    return make_shared<psData>(state,name,age,gender,race,illness,flee);
}
